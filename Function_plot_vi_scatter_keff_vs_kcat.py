...
dirIn = "input/Function/cleaned/"
dirOut = 'output/'
filelist = ["20170628-SsHelix-combinedreps", "20170717-SsHelix-plate1-combinedreps", "20170717-SsHelix-plate2-combinedreps"]
temp = 30
...
def createScatterWithLinearRegression(df, fout):
    ...
    ax1.set_xlabel(labeldict["kcat (1/s)"])
...
def createScatterErrorBarByGroups(velocity, velstd, group, protlist, proteinconc, extendx = 100):
    ...
    combinedTable["[Protein] (uM)"] = fit["[Protein] (uM)"]
    combinedTable["Vmax (uM/s)"] = fit["Vmax (uM/s)"].round(2).map(str) + u'\u00B1' + fit["Vmax_err"].round(2).map(str)
    combinedTable["kcat (1/s)"] = fit["kcat (1/s)"].round(2).map(str) + u'\u00B1' + fit["kcat_err"].round(2).map(str)
    combinedTable["Km (uM)"] = fit["Km (uM)"].round(2).map(str) + u'\u00B1' + fit["Km_err"].round(2).map(str)
    combinedTable["kcat/km (M-1*s-1)"] = fit["kcat/km (M-1*s-1)"].round(2).map(str) + u'\u00B1' + fit["kcat/km_err"].round(3).map(str)
    combinedTable["ddg-kcat (kcal/mol)"] = fit["ddg-kcat (kcal/mol)"].round(3).map(str)
    combinedTable["ddg-Keff (kcal/mol)"] = fit["ddg-Keff (kcal/mol)"].round(3).map(str)
    combinedTable.replace(to_replace = 'nan', value = '-', inplace = True)
    combinedTable = combinedTable.reindex(sorted(combinedTable.index, key=lambda x: orderdict[x]))
    combinedTable.columns = ["[Protein] (uM)", "Vmax (uM/s)", "kcat (1/s)" ,"Km (uM)", "kcat/km (M-1*s-1)","ddg-kcat (kcal/mol)", "ddg-Keff (kcal/mol)" ]
    combinedout = dirOut + group+'-initialvelocity-mm-formatted.csv'
    combinedTable.to_csv(combinedout, encoding='utf-8-sig', index=True)
    
    return combinedTable

extendx = 100
group = 'SsMutant-double-pooled'+extracaption
prots = ['SsWT', 'I45A','I45K',  'S70A',  'M73A', 'M73I', 'I107K', 'I45A_S70A', 'I45A_M73A',  ]
proteinconc=1

fittable = createScatterErrorBarByGroups(velocity, velstd, group, prots, proteinconc)
